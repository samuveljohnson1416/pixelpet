...
- **Web-swing:** press **Ctrl+Alt+Shift+W** (or pick **Web-swing here** in the menu) and it shoots a web to your mouse pointer, swings over on an arc, and lands with a little squash, a puff of dust and hearts. Every so often it also goes web-slinging on its own: two or three quick swings across the screen (never during a focus session, work, study or a movie).
...
- **Break buddy:** it keeps count of how long you've been on the computer without a break (a 5-minute pause resets it). After 45 minutes of non-stop use it says "45 min on screen. Time for a sip of water!" and drinks from a glass (`water = 45`). After 90 minutes it asks you to stand up and stretch, and stretches with you: it grows tall and reaches up one paw, then the other (`stretch = 90`). Every hour it holds up a little clock and tells you your screen time, with today's total (`screentime = 60`); the menu shows both at any time. The nudges wait for a free moment: never during a focus session, a movie, a meeting or a presentation, and never over something it's already saying. If the pet is hidden, water and stretch breaks arrive as notifications. Set any of the three to `0` to turn it off, or change them under **Break reminders** in Settings.
...
- Press Enter and it throws a rope at the place you were typing: the text caret in classic apps, or the focused field itself (a browser's address bar, a chat box, a search box) in browsers and modern apps, which it finds through UI Automation. Only if there's no focused field does it aim at the mouse pointer.
...
| `water` | `45` | Minutes of non-stop use before a water break (`0` = off) |
| `stretch` | `90` | Minutes of non-stop use before a stretch break (`0` = off) |
| `screentime` | `60` | Tells you your screen time every this many minutes of non-stop use (`0` = off) |
